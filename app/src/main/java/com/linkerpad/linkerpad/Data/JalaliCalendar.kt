...
class JalaliCalendar : Calendar {
    ...
    @JvmOverloads constructor(zone: TimeZone = TimeZone.getDefault(), aLocale: Locale = Locale.getDefault()) : super(zone, aLocale) {
        timeZone = zone
        val calendar = Calendar.getInstance(zone, aLocale)

        var yearMonthDate = YearMonthDate(calendar.get(YEAR), calendar.get(MONTH), calendar.get(DATE))
    ...
    constructor(year: Int, month: Int, dayOfMonth: Int, hourOfDay: Int,
                minute: Int) : this(year, month, dayOfMonth, hourOfDay, minute, 0, 0) {
    }

    constructor(year: Int, month: Int, dayOfMonth: Int, hourOfDay: Int,
                minute: Int, second: Int) : this(year, month, dayOfMonth, hourOfDay, minute, second, 0) {
    }

    internal constructor(year: Int, month: Int, dayOfMonth: Int,
                         hourOfDay: Int, minute: Int, second: Int, millis: Int) : super() {
        ...
        cal = GregorianCalendar(yearMonthDate.year, yearMonthDate.month, yearMonthDate.date, hourOfDay,
                minute, second)
    ...
    protected override fun computeTime() {
        ...
        if (!isTimeSet && !isTimeSeted) {
            ...
            cal.set(yearMonthDate.year, yearMonthDate.month, yearMonthDate.date, internalGet(HOUR_OF_DAY), internalGet(MINUTE), internalGet(SECOND))
            time = cal.getTimeInMillis()
            ...
            val yearMonthDate = jalaliToGregorian(YearMonthDate(internalGet(YEAR), internalGet(MONTH), internalGet(DAY_OF_MONTH)))
            cal!!.set(yearMonthDate.year, yearMonthDate.month, yearMonthDate.date, internalGet(HOUR_OF_DAY),
        ...
        when (field) {
            ...
            HOUR_OF_DAY, HOUR, MINUTE, SECOND, MILLISECOND, ZONE_OFFSET, DST_OFFSET -> {
                if (isSet(YEAR) && isSet(MONTH) && isSet(DATE) && isSet(HOUR) && isSet(HOUR_OF_DAY) &&
                    ...
                    cal = GregorianCalendar()
                    var yearMonthDate = jalaliToGregorian(YearMonthDate(internalGet(YEAR), internalGet(MONTH), internalGet(DATE)))
                    cal!!.set(yearMonthDate.year, yearMonthDate.month, yearMonthDate.date, internalGet(HOUR_OF_DAY), internalGet(MINUTE),
                            internalGet(SECOND))
                    cal!!.set(field, value)
                    yearMonthDate = gregorianToJalali(YearMonthDate(cal!!.get(YEAR), cal!!.get(MONTH), cal!!.get(DATE)))
    ...
    protected override fun computeFields() {
        val temp = isTimeSet
        if (!areFieldsSet) {
            ...
            //Day_of_Week
            super.set(DAY_OF_WEEK, dayOfWeek(jalaliToGregorian(YearMonthDate(fields[1], fields[2], fields[5]))))
            //***
    ...
    override fun add(field: Int, amount: Int) {
        ...
        if (field == MONTH) {
            ...
            val gc = GregorianCalendar(yearMonthDate.year, yearMonthDate.month, yearMonthDate.date,
                    get(HOUR_OF_DAY), get(MINUTE), get(SECOND))
            gc.add(field, amount)
            yearMonthDate = gregorianToJalali(YearMonthDate(gc.get(YEAR), gc.get(MONTH), gc.get(DATE)))
            super.set(YEAR, yearMonthDate.year)
            super.set(MONTH, yearMonthDate.month)
            super.set(DATE, yearMonthDate.date)
    ...
    companion object {
        var gregorianDaysInMonth = intArrayOf(31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31)
        var jalaliDaysInMonth = intArrayOf(31, 31, 31, 31, 31, 31, 30, 30, 30, 30, 30, 29)
        ...
        private val ONE_MINUTE = 60 * ONE_SECOND
        private val ONE_HOUR = 60 * ONE_MINUTE
        private val ONE_DAY = (24 * ONE_HOUR).toLong()
        internal val BCE = 0
        internal val CE = 1
        val AD = 1

        internal val MIN_VALUES = intArrayOf(BCE, // ERA
                1, // YEAR
                FARVARDIN, // MONTH
                1, // WEEK_OF_YEAR
                0, // WEEK_OF_MONTH
        ...
        internal val LEAST_MAX_VALUES = intArrayOf(CE, // ERA
                292269054, // YEAR
                ESFAND, // MONTH
                52, // WEEK_OF_YEAR
                4, // WEEK_OF_MONTH
        ...
        internal val MAX_VALUES = intArrayOf(CE, // ERA
                292278994, // YEAR
                ESFAND, // MONTH
                53, // WEEK_OF_YEAR
                6, // WEEK_OF_MONTH
        ...
        fun jalaliToGregorian(jalali: YearMonthDate): YearMonthDate {
            if (jalali.month > 11 || jalali.month < -11) {
                throw IllegalArgumentException()
            }
            ...
            jalaliDayNo = (365 * jalali.year + jalali.year / 33 * 8
                    + Math.floor(((jalali.year % 33 + 3) / 4).toDouble()).toInt())
            i = 0
        ...
        fun weekOfYear(dayOfYear: Int, year: Int): Int {
            ...
            when (dayOfWeek(JalaliCalendar.jalaliToGregorian(YearMonthDate(year, 0, 1)))) {
                2 -> dayOfYear++
                3 -> dayOfYear += 2
                4 -> dayOfYear += 3
                5 -> dayOfYear += 4
        ...
        fun dayOfWeek(yearMonthDate: YearMonthDate): Int {
            ...
            val cal = GregorianCalendar(yearMonthDate.year, yearMonthDate.month, yearMonthDate.date)
        ...
        fun isLeepYear(year: Int): Boolean {
            ...
            return if (year % 33 == 1 || year % 33 == 5 || year % 33 == 9 || year % 33 == 13 ||
